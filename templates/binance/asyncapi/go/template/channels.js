...
export default function ({ asyncapi, params }) {
  ...
  for (const entry of channelEntries) {
    ...
    content += `\t\t}\n`;
    content += `\t}\n`;
    content += `\tbaseClean := strings.TrimRight(base, "/")\n`;
    content += `\tvar query string\n`;
    content += `\tif idx := strings.Index(path, "?"); idx >= 0 {\n`;
    content += `\t\tquery = path[idx:]\n`;
    content += `\t\tpath = path[:idx]\n`;
    content += `\t}\n`;
    content += `\tif path != "" {\n`;
    content += `\t\tpath = "/" + strings.Trim(path, "/")\n`;
    content += `\t\tif path == "/" {\n`;
    content += `\t\t\tpath = ""\n`;
    content += `\t\t}\n`;
    content += `\t}\n`;
    content += `\tfull := baseClean + path + query\n`;
